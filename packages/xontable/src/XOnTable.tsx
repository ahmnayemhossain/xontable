...
export function XOnTable<Row extends Record<string, any>>(props: XOnTableProps<Row>) {
  ...
  const filters = useColumnFilters(columns, rows);
  ...
  const { data, active, setActive, getValue, updateCells, moveActive, rowCount, colCount, hasError, getError, setCellErrorView, undo, redo } = useTableModel<Row>({ columns: visibleColumns.map((v) => v.col), rows, rowFilter: filters.rowFilter, onChange });
  ...
  const startEditCell = React.useCallback((initial?: string) => { if (readOnly) return; ensureSelect(active.r, active.c, activeRow); startEdit(initial); }, [active.c, active.r, activeRow, ensureSelect, readOnly, startEdit]);
  ...
  const { clipRef, focusClipboard, onCopy, onPaste } = useClipboardCatcher({
    ...
    onPasteBlock: (block) => { if (readOnly) return; const updates: CellUpdate[] = []; for (let rOff = 0; rOff < block.length; rOff++) for (let cOff = 0; cOff < block[rOff].length; cOff++) { const r = active.r + rOff; const c = active.c + cOff; if (r < rowCount && c < colCount) updates.push({ r, c, value: block[rOff][cOff] }); } updateCells(updates, { type: "paste", cell: active }); updates.forEach((u) => validateSelect(u.r, u.c, u.value, data[u.r])); },
  });
  ...
  const openSelectAt = React.useCallback((r: number, c: number) => {
    ...
    const row = data[r]; ensureSelect(r, c, row);
    requestAnimationFrame(() => startEdit());
  }, [data, ensureSelect, focusClipboard, readOnly, selection, setActive, startEdit]);
  ...
  return (
    <div className={`xontable-wrap theme-${theme}`}>
      <textarea ref={clipRef} className="xontable-clip" aria-hidden="true" tabIndex={-1} onCopy={onCopy} onPaste={onPaste} onKeyDown={onGridKeyDownWithCopy} readOnly />
      <div
      ...
      {isEditing && editorRect && (
        <input ref={editorRef} className="xontable-editor" value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={onEditorKeyDown} onBlur={() => { commitEdit(); validateSelect(active.r, active.c, draft, activeRow); }} style={{ position: "fixed", left: editorRect.left, top: editorRect.top, width: editorRect.width, height: editorRect.height }} />
      )}
